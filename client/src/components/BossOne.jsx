...
export default function BossOne({
  ...
  const bossHealthDivs = [];
  for (let i = 0; i < bossOne.health; i++) {
    bossHealthDivs.push(<div key={i} className="health-bar-segment"></div>);
  }
  ...
  function handleFight(choice) {
    ...
    if (choice === "attack") {
      ...
      setBossOne({
        ...bossOne,
        friendly: bossOne.friendly + myUser.character.cha,
      });
      if (bossOne.friendly >= 9) {
        // Put the correct step for friendly victory screen here
        setCurrentStep(69);
      }
      setFightDescription([
        ...fightDescription,
